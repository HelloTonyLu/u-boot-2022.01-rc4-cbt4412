...
unsigned char Random_MAC_One(void);
void dm9000_autoinit_mac(struct bd_info *bis);
...
#else
	printascii("CONFIG_DM9000_NO_SROM is defined\r\n");
#endif
}
...
unsigned char Random_MAC_One(void)
{
    ...
    unsigned char rand_MAC;


    rand = rand * 1664525L + 1013904223L;
    rand_MAC = rand & 0x000000FF; //最低8位

    /*为了得到 ‘0~9, a ~ f, A ~ F’ 的值，

    if (a < 'A')
        rand_MAC = rand_MAC % 10 + 48;
    else if (a < 'F')
        rand_MAC = rand_MAC % 6 + 65;
    else if (a < 'a' || a > 'f')
        rand_MAC = rand_MAC % 6 + 97;
	*/
    return rand_MAC;
}
...
void dm9000_autoinit_mac(struct bd_info *bis)
{
	...
	/* fill device MAC address registers */
	unsigned int i = 0,oft;
	for (i = 0, oft = DM9000_PAR; i < 6; i++, oft++){
#ifdef CONFIG_SETRANDOM_MAC
		dev->enetaddr[i] = Random_MAC_One();
		if(i==0)dev->enetaddr[i] = 0x00;
		DM9000_iow(oft, dev->enetaddr[i]);
#else
		dev->enetaddr[i] = i*17;
		DM9000_iow(oft, dev->enetaddr[i]);
	...
	printf("MAC: %pM\n", dev->enetaddr);
	if (!is_valid_ethaddr(dev->enetaddr)) {
		printf("WARNING: dm9000_autoinit_mac Bad MAC address!\n");
	}
}
